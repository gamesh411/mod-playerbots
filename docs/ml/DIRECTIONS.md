# ML / PvP AI — active directions

Status legend: **active** · **paused** · **done (baseline)** · **next** · **superseded**

Update the status column when a track moves. Do not delete rows — mark **paused** / **superseded**.
...
| D3 | Terminal match backup | **done (baseline)** | `y = short + λ·(+1/−1)` with `match_id` | Logger v2/v3 schema | Extend to duels via D12 |
| D4 | ε-greedy exploration (legal queue actions) | **active (arena)** | Escape pure heuristic support in arenas | `MlExploreEpsilon=0.08` | Unchanged for arena; duels use D13 |
...
| D7 | Difficulty dial (reaction / knowledge / execution) | **next** | Profiles 1–10 gating features + delays | — | Spec config knobs after D12 ships |
...
| D9 | **Action-head / ranking net** | **active (primary arch)** | One forward per tick → scores over legal actions | DEC-011 | Design vocab + trainer ranking loss; keep Mode A baseline |
| D10 | Offline RL / imitation ranking (Mode B/C) | **paused** | True return maximization | — | After D9 vocab + D12 episodes |
| D11 | Generalist merge (distill specialists) | **paused** | One brain from arena/BG/PvE experts | — | Only after specialists beat heuristics |
| D12 | **Duel bracket (idle → duel)** | **active** | Config pairs, park, pair frequently, duel CSV | DEC-012; engine + `config.ps1` | Ship matcher + population filter; Arms↔Frost default |
